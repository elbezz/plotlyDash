...
app.layout = dbc.Container([
            ...
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H6('Percentil'),
                        html.Br(),
                        html.H3(id='content-Percentil', children="-")
            ...
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        ...
                        html.Br(),
                        html.H3(id='content-CIR-Conf', children="-")
                    ], style={"border-radius": "8%", "background": "PowderBlue", 'textAlign': 'center', 'margin-top': '20px', 'height': '120px'})
    ...
    dbc.Row([
        dbc.Col([  
            ...
            #                 # .add_trace(px.line(prcl_df, x="CIR-Configured", y="percentile").data[0])
            #         )      
            ], width=12),
            ]),
    ...
    dbc.Row([
        dbc.Col([    
            ...
            # dcc.Graph(
            #     figure=px.line(data_frame=prcl_df, x="percentile", y="CIR-Configured", template='ggplot2',
            #     width=1300, height=600, labels={"CIR_Conf": "CIR Configured"})
            #     .update_traces(mode='markers+lines')
            #     .add_trace(px.scatter(data_frame=prcl_df, x="percentile", y="CIR-Configured", text="CIR-Configured").data[0])
    ...
    dbc.Row([
        dbc.Col([
            ...
            dcc.Graph(
                figure=px.line(data_frame=regressiom_df, x="percentile", y="CIR-Configured", template='ggplot2',width=1300, height=600)
                .update_traces(mode='markers+lines')
                .add_trace(px.scatter(data_frame=regressiom_df, x="percentile", y="CIR-Configured", text="CIR-Configured").data[0])
                .update_traces(textposition="bottom right")
        ...
        dbc.Col([     
            ...
            dt.DataTable(
                ...
                for i in regressiom_df[["CIR-Configured", "percentile"]]],
...
#-----------------------------------------------------------------------------------------
  ...
  .update_layout(title='', title_x=0.5, showlegend=False,).update_layout(yaxis=dict(tickfont=dict(size=12)), xaxis=dict(tickfont=dict(size=12)), font=dict(family="Courier New, monospace", size=10, color="black")).data[0])
    ...
    return percentil, max_cr_conf, trace1, trace2, dffLine.to_dict('records')
