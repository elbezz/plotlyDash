...
def update_small_cards(option_slctd, option_slctd2):
    ...
    dffLine['diff'] = round((dffLine['CIR_Conf'] - dffLine['percentile']) /
                            dffLine['CIR_Conf']*100, 2).astype(str) + '%'
    # dffLine['diff'] = dffLine['percentile'].astype(str) +"-"+ dffLine['diff']
...
# -----------------------------------------------------------------------------------------
    ...
    trace5 = px.line(data_frame=cir_conf_time_df, x=cir_conf_time_df.index, y="CIR_Conf", template='plotly_dark', width=1490, height=500, labels={"CIR_Conf": "CIR Configured"}).update_traces(mode='markers+lines').add_trace(px.area(data_frame=planning_BW_acquisition, x=planning_BW_acquisition.index, y="predicted").data[0]).update_traces(textposition="bottom right", fillcolor='#32a852').add_trace(px.line(data_frame=planning_BW_acquisition_up_to_max).data[0]).add_trace(px.line(data_frame=sales_forecast_df, x=sales_forecast_df.index, y="predicted", text="predicted").data[0]).update_layout(
