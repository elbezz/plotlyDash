...
prcl_df = dataset.groupby('CIR_Conf')['CIR_Alloc'].agg(
    [percentil95]).reset_index()
prcl_df['percentil95'] = round(prcl_df['percentil95'], 2)
...
x_inv = ConfPrclDataset[["percentil95"]]
...
# Prédire maintenant CIR_Conf quand percentil95 = CIR_Phy avec le modèle inversé qu'on vient de mettre en place
...
app.layout = dbc.Container([
    dbc.Row([
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H3('ATS Overbooking system'),
    ...
    dbc.Row([
        dbc.Col([  
            dcc.Graph(figure=px.violin(data_frame=dataset,x="CIR_Conf",y="CIR_Alloc",orientation="v",box=True,
                            color='CIR_Conf',log_x=True,log_y=True,labels={"CIR_Conf": "CIR Configured",
                                    "CIR_Alloc": "CIR Allocated"},
                            width=1500,height=600,template='ggplot2',animation_frame='CIR_Conf',
                            range_x=[10, 300],range_y=[5, 70],)
                            .update_layout(yaxis=dict(tickfont=dict(size=1)), xaxis=dict(tickfont=dict(size=1)), font=dict(family="Courier New, monospace",size=10,color="black"))
                            .add_trace(px.line(prcl_df, x="CIR-Configured", y="percentil95").data[0])
                    )      
            ], width=12),
            ]),
    ...
    dbc.Row([
        dbc.Col([        
            dcc.Graph(
                figure=px.line(data_frame=prcl_df, x="percentil95", y="CIR-Configured", template='ggplot2',
                width=1300, height=600, labels={"CIR_Conf": "CIR Configured"})
                .update_traces(mode='markers+lines')
                .add_trace(px.scatter(data_frame=prcl_df, x="percentil95", y="CIR-Configured", text="CIR-Configured").data[0])
                .update_traces(textposition="bottom right")
                .update_layout(title='',title_x=0.5,showlegend=False,).update_layout(
        ...
        dbc.Col([
                ...
                html.Br(),   
                    dt.DataTable(
                        id='tbl', data=prcl_df.to_dict('records'),columns=[{"name": i, "id": i}
    ...
    dbc.Row([
        dbc.Col([
            dcc.Graph(
                figure=px.line(data_frame=regressiom_df, x="percentil95", y="CIR-Configured", template='ggplot2',width=1300, height=600)
                .update_traces(mode='markers+lines')
                .add_trace(px.scatter(data_frame=regressiom_df, x="percentil95", y="CIR-Configured", text="CIR-Configured").data[0])
                .update_traces(textposition="bottom right")
                .update_layout(title='',title_x=0.5,showlegend=False,)
                .update_layout(yaxis=dict(tickfont=dict(size=12)),xaxis=dict(tickfont=dict(size=12)),
                font=dict(family="Courier New, monospace",size=10,color="black")))
            ], width=10),
        ...
        dbc.Col([     
            ...
            html.Br(),
            dt.DataTable(
                id='tblPred', data=regressiom_df.to_dict('records'),columns=[{"name": i, "id": i}
                for i in regressiom_df[["CIR-Configured", "percentil95"]]],
    ...
    dbc.Row([
        ...
        dbc.Col([
            dbc.Card([
                ...
                dbc.CardBody([
                    html.H3('Left'),
                    html.H5(id='content-Left', children="000")
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H3('Remaining Time'),
                    html.H5(id='content-RemainingTime', children="000")
                ], style={"border-radius": "8%", "background": 'rgba(50, 250, 120, 1.0)', 'inverse': 'True', 'textAlign': 'center', 'margin-top': '20px', 'height': '120px'})
            ], color="light", outline=True),
        ], width=2),
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H3('Overbooking Ratio'),
                    html.H5(id='content-OverbookingRatio', children="000")
                ], style={"border-radius": "8%", "background": 'rgba(50, 250, 120, 1.0)', 'inverse': 'True', 'textAlign': 'center', 'margin-top': '20px', 'height': '120px'})
